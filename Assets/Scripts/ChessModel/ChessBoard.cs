...
namespace ChessModel
{
    public class ChessBoard
    {
        ...
        // --- [NEW] Tham chiếu đến luật Tiêu Thổ ---
        public ScorchedEarthRules ScorchedRules;

        // Khởi tạo: Tạo mảng 64 ô, lưu tham chiếu BoardManager, và xếp quân cờ vào vị trí ban đầu.
        public ChessBoard(BoardManager boardManager)
        {
            ...
            // --- [NEW] Lấy component luật từ BoardManager ---
            ScorchedRules = boardManager.GetComponent<ScorchedEarthRules>();

            InitializeBoard();
        }

        // --- [NEW] Helper kiểm tra ô có bị chặn (Hố đỏ) không ---
        // Hàm này được Piece.cs gọi để chặn đường đi của Xe/Tượng/Hậu
        public bool IsTileBlocked(int position)
        {
            if (ScorchedRules != null && ScorchedRules.IsPathBlocked(position))
                return true;
            return false;
        }
        ...
        // Chức năng: Thực hiện một nước đi (Move).
        ...
        // Chức năng: Hoàn tác nước đi cuối cùng (Undo).
        public void Unplay()
        {
        ...
        // Chức năng: Hàm phụ trợ để hoán đổi vị trí 2 quân cờ trong mảng Board[].
        ...
        // Chức năng: Lấy danh sách tất cả các nước đi hợp lệ từ một vị trí cụ thể.
        public Piece GetPiece(int position)
        {
            return Board[position];
        }

        // Chức năng: Tính điểm lợi thế của bàn cờ (Dùng cho AI).
        public List<Move> GetMoveFromPosition(int position)
        {
            var piece = Board[position];
            var moves = piece.Color == NextToPlay ? piece.GetLegalMoves() : new List<Move>();

            // --- [NEW] Lọc bỏ các nước đi rớt xuống hố (Màu đỏ) ---
            moves.RemoveAll(m => IsTileBlocked(m.EndPosition));

            return moves;
        }

        // Chức năng: Cập nhật tổng giá trị quân cờ của mỗi bên (Đếm xem còn bao nhiêu quân).
        ...
        // Chức năng: Kiểm tra xem quân Tốt có đi đến cuối bàn cờ để phong cấp không.
        ...
        // Chức năng: Xử lý logic Nhập thành (Castling).
        ...
        // Chức năng: Lấy tất cả các nước đi hợp lệ của một phe (Dùng cho AI để duyệt cây nước đi).
        public List<Move> GetAllLegalMoves(ChessColor color)
        {
            var moves = Board.Where(piece => piece.Color == color).SelectMany(piece => piece.GetLegalMoves()).ToList();
            // --- [NEW] Lọc bỏ các nước đi vào hố cho AI ---
            moves.RemoveAll(m => IsTileBlocked(m.EndPosition));
            return moves;
        }

        // Chức năng: Kiểm tra xem Vua của phe 'color' có đang bị chiếu hay không.
        ...
        // Chức năng: Kiểm tra Chiếu bí (CheckMate).
        ...
        // Chức năng: Kiểm tra Hòa cờ(Draw).
        public bool IsDraw()
        {
        ...
        // Chức năng: Kiểm tra ô đó có trống không.
        ...
        // Chức năng: Thiết lập bàn cờ mới (Reset).
